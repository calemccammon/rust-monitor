...
pub fn draw_ui(
    ...
    terminal.draw(|f| {
        ...
        let chunks = Layout::default()
            ...
            .constraints([Constraint::Length(3), Constraint::Length(3), Constraint::Length(3), Constraint::Min(1)].as_ref())
            .split(size);
        ...
        if let Some((util, temp, name)) = gpu_info {
            ...
            let short = if first_line.len() > 40 { format!("{}...", &first_line[..37]) } else { first_line.to_string() };
        ...
        for (i, (pid, name, cpu, delta, mem)) in proc_list.iter().enumerate() {
            ...
            let row = Row::new(vec![pid.to_string(), name.clone(), cpu_text, delta_text, mem_text]);
            let row = if i == selected {
                row.style(Style::default().fg(Color::Black).bg(Color::LightGreen).add_modifier(Modifier::BOLD))
            } else { row };
            rows.push(row);
        }
        ...
        let table = Table::new(rows)
            ...
            .widths(&[Constraint::Length(8), Constraint::Percentage(45), Constraint::Length(8), Constraint::Length(8), Constraint::Length(12)]);
        f.render_widget(table, chunks[3]);

        let mut help_text = "q:quit  p:pause  s:cycle sort  ↑/↓:select  k:kill (confirm)".to_string();
        if let Some(err) = last_gpu_error {
            let first_line = err.lines().next().unwrap_or("");
            let short = if first_line.len() > 60 { format!("{}...", &first_line[..57]) } else { first_line.to_string() };
            help_text.push_str("    GPU Err: ");
            help_text.push_str(&short);
        }
        let help = Paragraph::new(help_text)
            .block(Block::default().borders(Borders::ALL).title("Keys"));
        let footer_area = Layout::default().direction(Direction::Vertical).constraints([Constraint::Min(1), Constraint::Length(3)]).split(chunks[3])[1];
        ...
        if let Some(kpid) = confirming_kill {
            ...
            let area = ratatui::layout::Rect { x: size.width/4, y: size.height/3, width: size.width/2, height: 3 };
